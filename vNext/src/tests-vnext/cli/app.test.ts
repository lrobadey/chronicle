...
describe('CLI app', () => {
  ...
  it('renders live current thoughts and trace tool events without raw payloads in summary mode', () => {
    const thoughts = renderDebugEvent({
      type: 'llm.response.received',
      agent: 'systems_designer',
      status: 'completed',
      responseId: 'resp-systems-1',
      toolCalls: 1,
      reasoningHeadings: ['Reading the room', 'Checking mechanics'],
    }, 'summary');
    assert.equal(thoughts, '[thoughts] systems designer: Reading the room; Checking mechanics\n');

    const call = renderDebugEvent({
      type: 'trace.tool.called',
      agent: 'systems_designer',
      tool: 'inspect_local_affordances',
      callId: 'call-systems-1',
      callIndex: 1,
      callCount: 1,
      input: { actorId: 'player-1', verbose: true },
    }, 'summary');
    assert.equal(call, '[tool] systems designer 1/1 inspect local affordances\n');
    assert.ok(!call.includes('player-1'));

    const result = renderDebugEvent({
      type: 'trace.tool.result',
      agent: 'systems_designer',
      tool: 'emit_systems_result',
      callId: 'call-systems-2',
      callIndex: 1,
      callCount: 1,
      output: { ok: true, summary: 'Prepared observation packet.' },
      ok: true,
    }, 'summary');
    assert.equal(result, '[tool] systems designer 1/1 emit systems result complete\n');
    assert.ok(!result.includes('Prepared observation packet.'));
  });

  it('renders live trace tool payloads in raw mode', () => {
    const raw = renderDebugEvent({
      type: 'trace.tool.called',
      agent: 'world_designer',
      tool: 'inspect_world_summary',
      callId: 'call-world-1',
      callIndex: 1,
      callCount: 2,
      input: { focus: 'dockside anomaly' },
    }, 'raw');

    assert.ok(raw.includes('[tool] world designer 1/2 inspect_world_summary call=call-world-1'));
    assert.ok(raw.includes('"focus": "dockside anomaly"'));
  });
  ...
  it('renders mechanics tool summaries and raw payloads compactly', () => {
    ...
    const failedMechanics = renderDebugEvent({
      ...
      output: {
        ...
        candidateEvents: [],
        confidence: 0,
        debug: { selectedModel: 'gpt-5.5', usedFallback: true, failureReason: 'invalid_function_output' },
      },
      ok: true,
    }, 'summary');
    ...
    assert.ok(resolveRaw.includes('model=gpt-5.5'));
