...
import { createChronicleServer, resolveServerApiKey, resolveStaticDir } from '../../server';
...
describe('vNext API compatibility', () => {
  it('resolves server API keys with request and standard environment precedence', () => {
    const previousOpenAI = process.env.OPENAI_API_KEY;
    const previousVite = process.env.VITE_OPENAI_API_KEY;
    try {
      process.env.OPENAI_API_KEY = 'server-key';
      process.env.VITE_OPENAI_API_KEY = 'vite-key';
      assert.equal(resolveServerApiKey('request-key'), 'request-key');
      assert.equal(resolveServerApiKey(), 'server-key');
      delete process.env.OPENAI_API_KEY;
      assert.equal(resolveServerApiKey(), 'vite-key');
    } finally {
      if (previousOpenAI == null) delete process.env.OPENAI_API_KEY;
      else process.env.OPENAI_API_KEY = previousOpenAI;
      if (previousVite == null) delete process.env.VITE_OPENAI_API_KEY;
      else process.env.VITE_OPENAI_API_KEY = previousVite;
    }
  });
  ...
  it('resolves the static directory from the server module path instead of cwd', () => {
    ...
    assert.equal(resolveStaticDir(), path.resolve(process.cwd(), 'src', 'dist'));
  });
