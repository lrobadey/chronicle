...
export function resolveServerApiKey(requestApiKey?: string): string | undefined {
  return requestApiKey || process.env.OPENAI_API_KEY || process.env.VITE_OPENAI_API_KEY || undefined;
}
...
export function resolveStaticDir(moduleUrl?: string) {
  if (moduleUrl) {
    return path.resolve(path.dirname(fileURLToPath(moduleUrl)), 'dist');
  }
  // Vite uses `src` as its project root, so its default output is `src/dist`.
  // Package scripts run from the repository root; keeping this explicit avoids
  // accidentally serving an unrelated root-level dist directory.
  return path.resolve(process.cwd(), 'src', 'dist');
}
...
function serveStatic(res: http.ServerResponse, urlPath: string, staticDir: string): boolean {
  ...
  return http.createServer(async (req, res) => {
    ...
    try {
      if (urlPath === '/api/init' && method === 'POST') {
        ...
        if (!stream) {
          const result = await engine.initSession({ sessionId, worldId, apiKey: resolveServerApiKey(apiKey) });
          sendJSON(res, 200, {
        ...
        const result = await engine.initSession({
          sessionId,
          worldId,
          apiKey: resolveServerApiKey(apiKey),
          stream: {
      ...
      if (urlPath === '/api/turn' && method === 'POST') {
        ...
        if (!stream) {
          const result = await engine.runTurn({
            ...
            playerId: 'player-1',
            playerText,
            apiKey: resolveServerApiKey(asOptionalString(body.apiKey)),
      ...
      if (method === 'GET' && serveStatic(res, urlPath, staticDir)) return;
