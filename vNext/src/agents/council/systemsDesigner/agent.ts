...
import { emitTraceToolCalled, isFunctionCallItem, pushLLMTrace, pushToolTrace } from '../../llm/trace';
...
async function runSystemsDesignerLoop(
  ...
  for (let index = 0; index < 4; index += 1) {
    ...
    pushLLMTrace(params.trace, {
      ...
      reasoningHeadings: response.reasoningHeadings,
      previousResponseId,
    ...
    for (const call of toolCalls) {
      ...
      const toolInput = parsed.ok ? parsed.value : call.arguments;
      emitTraceToolCalled(params.trace, {
        tool: call.name,
        input: toolInput,
        agent: 'systems_designer',
        iteration: index + 1,
        callId,
        callIndex: nextInput.length + 1,
