...
export class OpenAIClient implements LLMClient {
  async responsesCreate(params: ResponseCreateParams): Promise<ResponseCreateResult> {
    ...
    const payload = {
      ...
      reasoning: withDefaultReasoningSummary(params.reasoning ?? { effort: 'low' }),
...
function toResult(response: any): ResponseCreateResult {
  const output = (response.output ?? []) as unknown as ResponseCreateResult['output'];
  const reasoningHeadings = extractReasoningHeadings(output);
  return {
      id: response.id,
      ...
      reasoningHeadings: reasoningHeadings.length ? reasoningHeadings : undefined,
  };
}

function withDefaultReasoningSummary(reasoning: ResponseCreateParams['reasoning']) {
  if (!reasoning || reasoning.summary) return reasoning;
  if (!reasoning.effort || reasoning.effort === 'none') return reasoning;
  return { ...reasoning, summary: 'auto' as const };
}

export function extractReasoningHeadings(output: ResponseCreateResult['output']): string[] {
