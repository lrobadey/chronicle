...
export function thinkingPhaseForDebugEvent(event: DebugEvent): ThinkingPhase | null {
  switch (event.type) {
    ...
    case 'tool.called':
    case 'tool.result':
    case 'trace.tool.called':
    case 'trace.tool.result':
    case 'npc.started':
    case 'npc.completed':
...
function renderDebugSummary(event: DebugEvent): string {
  switch (event.type) {
    ...
    case 'llm.response.received':
      return summarizeLLMResponse(event);
    case 'tool.called':
      return summarizeToolCall(event);
    case 'tool.result':
      return summarizeToolResult(event);
    case 'trace.tool.called':
      return summarizeTraceToolCall(event);
    case 'trace.tool.result':
      return summarizeTraceToolResult(event);
    case 'event.accepted':
...
function renderRawDebugSummary(event: DebugEvent): string {
  ...
  if (event.type === 'llm.response.received') {
    const toolCalls = typeof event.toolCalls === 'number' ? ` toolCalls=${event.toolCalls}` : '';
    const responseId = event.responseId ? ` response=${event.responseId}` : '';
    return `[thoughts] ${formatAgentLabel(event.agent, event.specialistType)} status=${event.status || 'unknown'}${toolCalls}${responseId}`;
  }
  ...
  if (event.type === 'trace.tool.called') {
    return `[tool] ${formatTraceToolPrefix(event)} ${event.tool}${event.callId ? ` call=${event.callId}` : ''} ${summarizeToolInput(event.tool, event.input, 'raw')}`.trimEnd();
  }
  if (event.type === 'trace.tool.result') {
    return `[tool] ${formatTraceToolPrefix(event)} result ${event.tool}${event.callId ? ` call=${event.callId}` : ''} ${summarizeToolOutput(event.tool, event.output, 'raw')}`.trimEnd();
  }
  ...
  switch (event.type) {
    ...
    case 'llm.response.received':
      return {
        ...
        status: event.status,
        toolCalls: event.toolCalls,
        reasoningHeadings: event.reasoningHeadings,
        error: event.error,
      };
    case 'tool.called':
      return {
...
function summarizeLLMResponse(event: Extract<DebugEvent, { type: 'llm.response.received' }>): string {
  const headings = (event.reasoningHeadings || []).map(heading => heading.trim()).filter(Boolean);
  if (headings.length) {
    return `[thoughts] ${formatAgentLabel(event.agent, event.specialistType)}: ${headings.join('; ')}`;
  }
  const toolCalls = typeof event.toolCalls === 'number' ? ` toolCalls=${event.toolCalls}` : '';
  return `[llm] ${formatAgentLabel(event.agent, event.specialistType)} status=${event.status || 'unknown'}${toolCalls}`;
}
...
function summarizeTraceToolCall(event: Extract<DebugEvent, { type: 'trace.tool.called' }>): string {
  const detail = summarizeToolInput(event.tool, event.input, 'summary');
  const prefix = formatTraceToolPrefix(event);
  return detail ? `[tool] ${prefix} ${detail}` : `[tool] ${prefix} ${formatToolName(event.tool, 'summary')}`;
}

function summarizeTraceToolResult(event: Extract<DebugEvent, { type: 'trace.tool.result' }>): string {
  const detail = summarizeToolOutput(event.tool, event.output, 'summary');
  const prefix = formatTraceToolPrefix(event);
  return detail ? `[tool] ${prefix} ${detail}` : `[tool] ${prefix} ${formatToolName(event.tool, 'summary')} complete`;
}

function formatTraceToolPrefix(event: { agent?: string; stage?: string; callIndex?: number; callCount?: number }): string {
  const owner = event.agent ? formatAgentLabel(event.agent) : event.stage ? event.stage.replace(/_/g, ' ') : 'trace';
  const index = typeof event.callIndex === 'number' && typeof event.callCount === 'number'
    ? ` ${event.callIndex}/${event.callCount}`
    : '';
  return `${owner}${index}`;
}

function formatAgentLabel(agent: string, specialistType?: string): string {
  const base = agent.replace(/_/g, ' ');
  return specialistType ? `${base}:${specialistType}` : base;
}
